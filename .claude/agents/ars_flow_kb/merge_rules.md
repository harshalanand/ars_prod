...
- 2026-07-17 — **KB NAMING CLASH: two different "Merge Rules".** (a) The **Merge Rules module / endpoint** (`api/v1/endpoints/merge_rules.py` + `services/derived_masters.py`) = RNG_SEG-style dimension merging: flat `ARS_MERGE_RULES(source_col, source_value, target_value, agg)` collapses raw tiers into merged buckets (e.g. `E,V→EV`; `P,SP→PSP`) and rebuilds `Master_CONT_MERGE_<col>` + the `MERGE_<col>` grid-hierarchy columns. Canonical spec = manual `merge.md`. (b) The **OPT_TYPE classification (RL/TBC/TBL)** lives in the rule engine (`rule_engine_per_opt.py`), NOT in merge_rules.py — that content in this file's header is about the allocation engine, defer to `rule_ars`. Don't conflate the two. Live `ARS_MERGE_RULES`: 4 active rows, source_col=RNG_SEG, agg=SUM, → buckets EV (E,V) and PSP (P,SP).
