...
- **2026-07-17 (from live DB)** — **DOC DRIFT: MJ_RNG_SEG is NOT the Primary grid anymore.** Live `ARS_GRID_BUILDER` config: `grid_group='Primary'` = `MJ` (WERKS,MAJ_CAT) + `MJ_MERGE_RNG_SEG` (WERKS,MAJ_CAT,MERGE_RNG_SEG). `MJ_RNG_SEG` (raw tier) is now `grid_group='Secondary'`, `sec_cap_applicable=1 pct=130`, AND it is the single `use_for_opt_sale=1` grid (source of listing PER_OPT_SALE). The manual `grid.md` dossier lines 13-14/57 still say "MJ_RNG_SEG is the Primary grid" — that is stale. Why: the merged-tier grid replaced the raw-tier grid as the budget baseline; raw RNG_SEG demoted to a sec-cap + opt-sale source. How to apply: when reasoning about Primary budget, use MJ / MJ_MERGE_RNG_SEG.
- **2026-07-17 (from live DB)** — **Actual sec_cap_applicable grids** = `MJ_RNG_SEG`, `MJ_FIT`, `MJ_M_YARN_02` (all pct=130). `MJ_FAB`, `MJ_CLR`, `MJ_M_VND_CD`, `MJ_WEAVE_2` are Active Secondary but `sec_cap_applicable=0` (no cap). `MJ_MACRO_MVGR` and `MJ_MICRO_MVGR` are **Inactive**. So docs citing MJ_FAB / MJ_MICRO_MVGR as the canonical sec-cap examples are illustrative, not the live cap set. `ARS_GRID_HIERARCHY` live columns = MAJ_CAT, MERGE_RNG_SEG, RNG_SEG, M_YARN_02, WEAVE_2, FAB, CLR, M_VND_CD, FIT, SZ_APPLICABLE.
- **2026-07-17** — Sec-cap eligibility is enforced at CRUD in `grid_builder.create_grid` (L1680-1689) / `update_grid` (L1764-1772): `sec_cap_applicable` is force-zeroed unless `grid_group='Secondary'` AND `pivot_only=0`. Only ONE grid may have `use_for_opt_sale=1` (others auto-unset, L1678-1679 / L1758-1759).
