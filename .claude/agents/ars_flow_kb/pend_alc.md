...
- 2026-07-08 — parked_history.py module docstring (:12-19) claims SIX tables are parked/promoted (incl. ARS_MSA_GEN_ART/VAR_ART) but `_SNAPSHOT_TARGETS` (:59-84) configures only FOUR — MSA GEN_ART/VAR_ART are neither parked nor promoted to history on approve. Why: don't trust the docstring when auditing approve writes.
- 2026-07-08 — Approve DOES write grid tables: `apply_pend_alc_delta` auto-discovers every active `dbo.ARS_GRID_MJ*` (pend_alc_service.py:4813) and applies `PEND_ALC += qty, STK_TTL += qty` (:5020-5032), inserting missing rows on +1 (:5467-5470). Keyed on WERKS = PEND_ALC.ST_CD (destination store). Why: grid PEND is delta-maintained at approve, not only at grid rebuild.
...
- 2026-07-08 — ~~supersedes 2026-06-13 adhoc-close entry~~: adhoc close NOW calls `bootstrap_msa_pend_sync` after closing rows (`pend_alc_service.py:3083`, non-fatal on failure). MSA PEND no longer stays inflated until the next Generate. Why: menu/endpoint audit found the fix landed after the original KB note.
