...
ATTACHMENT_TABLE = "PT_ATTACHMENT"
MAX_DEPTH = 3  # PROJECT → SUB_PROJECT → TASK

# Attachment storage — persisted on Azure App Service under the backend dir
ATTACHMENT_DIR = Path(__file__).resolve().parents[4] / "pt_attachments"
...
def _ensure_tables(engine):
    ...
    with engine.connect() as c:
        ...
        # PT_ATTACHMENT — files attached to a project (sample/reference data)
        _run(c, f"""
            IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='{ATTACHMENT_TABLE}')
            BEGIN
                ...
                CREATE INDEX IX_{ATTACHMENT_TABLE}_PROJECT
                    ON [{ATTACHMENT_TABLE}](PROJECT_ID, IS_DELETED);
            END
        """)
...
try:
    ...
    logger.debug("[pt] PT_PROJECT / PT_ACTIVITY_LOG tables ready")
...
def list_projects(
    ...
    where, params = _build_filter_sql(
        ...
        archived=archived, overdue=overdue, open_only=open_only,
    )
    eng = get_data_engine()
    with eng.connect() as c:
...
def _attachment_disk_path(stored_name: str) -> Path:
    # Defensive: stored_name is generated by us, but never trust it for traversal
    safe = re.sub(r"[^A-Za-z0-9._-]", "", stored_name)
    return ATTACHMENT_DIR / safe


@router.post("/projects/{pid}/attachments", response_model=APIResponse)
async def upload_attachment(pid: int,
                            file: UploadFile = File(...),
                            current_user: User = Depends(get_current_user)):
    """Attach a file (sample / reference data) to a project."""
    ext = _safe_ext(file.filename)
    if ext not in ATTACHMENT_ALLOWED_EXT:
        raise HTTPException(400,
            f"File type {ext or '(none)'} not allowed. Allowed: {sorted(ATTACHMENT_ALLOWED_EXT)}")
    ...
    if size > ATTACHMENT_MAX_BYTES:
        raise HTTPException(400,
            f"File too large ({size/1024/1024:.1f} MB). Max {ATTACHMENT_MAX_BYTES//1024//1024} MB.")

    eng = get_data_engine()
    actor = current_user.username
    with eng.connect() as c:
        proj = c.execute(text(f"SELECT PROJECT_ID, NAME, IS_ARCHIVED FROM [{PROJECT_TABLE}] WHERE PROJECT_ID=:i"),
                         {"i": pid}).fetchone()
        if not proj:
            raise HTTPException(404, f"Project {pid} not found")
        if proj.IS_ARCHIVED:
            raise HTTPException(400, "Cannot attach to an archived project")

        ATTACHMENT_DIR.mkdir(parents=True, exist_ok=True)
        stored_name = f"{pid}_{uuid.uuid4().hex}{ext}"
        disk_path = _attachment_disk_path(stored_name)
        try:
            disk_path.write_bytes(content)
        except OSError as e:
            logger.exception(f"[pt] Failed writing attachment to disk: {e}")
        ...
        try:
            result = c.execute(text(f"""
                INSERT INTO [{ATTACHMENT_TABLE}]
                    (PROJECT_ID, ORIGINAL_NAME, STORED_NAME, FILE_SIZE, MIME_TYPE, UPLOADED_BY)
                OUTPUT INSERTED.ATTACHMENT_ID
                VALUES (:pid, :on, :sn, :sz, :mt, :ub)
            """), {
                "pid": pid, "on": file.filename, "sn": stored_name,
                "sz": size, "mt": mime, "ub": actor,
            })
            aid = int(result.scalar())
            _log_activity(c, pid, "ATTACHMENT_ADDED", actor,
                          details=f"Attached '{file.filename}' ({size} bytes)")
            c.commit()
        except Exception:
            # Roll back the on-disk file if DB insert failed
            try: disk_path.unlink(missing_ok=True)
            except Exception: pass
            raise

    logger.info(f"[pt] {actor} attached {file.filename} ({size}B) to project {pid}")
    return APIResponse(success=True, message="Uploaded", data={
        "attachment_id": aid,
...
@router.get("/projects/{pid}/attachments", response_model=APIResponse)
def list_attachments(pid: int, current_user: User = Depends(get_current_user)):
    eng = get_data_engine()
    with eng.connect() as c:
        rows = c.execute(text(f"""
            SELECT ATTACHMENT_ID, PROJECT_ID, ORIGINAL_NAME, FILE_SIZE, MIME_TYPE,
                   UPLOADED_BY, UPLOADED_AT
            FROM [{ATTACHMENT_TABLE}]
            WHERE PROJECT_ID = :i AND IS_DELETED = 0
...
def download_attachment(aid: int, current_user: User = Depends(get_current_user)):
    eng = get_data_engine()
    with eng.connect() as c:
        row = c.execute(text(f"""
            SELECT ORIGINAL_NAME, STORED_NAME, MIME_TYPE
            FROM [{ATTACHMENT_TABLE}]
            WHERE ATTACHMENT_ID = :i AND IS_DELETED = 0
        """), {"i": aid}).fetchone()
        if not row:
            raise HTTPException(404, "Attachment not found")
    disk_path = _attachment_disk_path(row.STORED_NAME)
...
@router.delete("/attachments/{aid}", response_model=APIResponse)
def delete_attachment(aid: int, current_user: User = Depends(get_current_user)):
    eng = get_data_engine()
    actor = current_user.username
    with eng.connect() as c:
        row = c.execute(text(f"""
            SELECT ATTACHMENT_ID, PROJECT_ID, ORIGINAL_NAME, STORED_NAME
            FROM [{ATTACHMENT_TABLE}]
            WHERE ATTACHMENT_ID = :i AND IS_DELETED = 0
        """), {"i": aid}).fetchone()
        if not row:
            raise HTTPException(404, "Attachment not found")
        c.execute(text(f"UPDATE [{ATTACHMENT_TABLE}] SET IS_DELETED=1 WHERE ATTACHMENT_ID=:i"),
                  {"i": aid})
        _log_activity(c, row.PROJECT_ID, "ATTACHMENT_REMOVED", actor,
                      details=f"Removed '{row.ORIGINAL_NAME}'")
        c.commit()

    # Best-effort disk cleanup — DB soft-delete is the source of truth
    try:
        _attachment_disk_path(row.STORED_NAME).unlink(missing_ok=True)
    except Exception as e:
        logger.warning(f"[pt] Could not unlink attachment file {row.STORED_NAME}: {e}")
