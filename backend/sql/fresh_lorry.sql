/* ============================================================================
   FRESH LORRY — allocation report from ARS_ALLOC_HISTORY for the LAST RUN
   (latest SESSION_ID in ARS_ALLOC_HISTORY).

   One row per store × variant-article × size (alloc waves/rounds summed).
   Only lines with activity (ALLOC_QTY or HOLD_QTY <> 0) are returned.
